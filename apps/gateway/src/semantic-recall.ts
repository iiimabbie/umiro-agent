...
export class SemanticRecallProvider implements ContextProvider {
  ...
  async load(request: Parameters<ContextProvider["load"]>[0]) {
    ...
    try {
      ...
      const hits = await this.search.semanticSearch(vector, this.embedder.model, 5, request.execution.authority.visibility, { ...(currentConversation ? { excludeConversationId: currentConversation } : {}), minSimilarity: 0.68 });
