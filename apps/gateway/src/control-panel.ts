...
const HTML = `<!doctype html><html lang="zh-Hant"><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>ümiro 控制台</title><style>body{font:15px system-ui;max-width:1100px;margin:30px auto;padding:0 16px;background:#111;color:#eee}button,input,select,textarea{font:inherit}input,textarea,select{box-sizing:border-box;width:100%;background:#1d1d1d;color:#eee;border:1px solid #555;padding:8px}textarea{height:360px}button{width:auto;padding:8px 14px;margin:6px 4px 6px 0}section{border:1px solid #444;padding:16px;margin:16px 0}small{color:#aaa}.files button{font-size:13px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}.schedule,.plugin,.run,.channel{border-top:1px solid #444;padding:8px 0}pre{white-space:pre-wrap}</style><h1>ümiro V2 控制台</h1><section><label>Web UI Token<input id="token" type="password" autocomplete="off"></label><button id="connect">連線</button><span id="state"></span><pre id="runtime"></pre><h3>Secrets 狀態</h3><pre id="secrets"></pre></section><section><h2>Discord 頻道</h2><button id="refreshChannels">重新取得名稱</button><small>名稱直接向 Discord 取得；頻道更名後會自動更新。ID 僅作為穩定識別。</small><div id="channels"></div></section><section><h2>最近 Runs</h2><button id="refreshRuns">重新整理</button><div id="runs"></div><pre id="runDetail"></pre></section><section><h2>設定</h2><h3>API 可用模型</h3><pre id="models"></pre><div id="help"></div><textarea id="config"></textarea><button id="saveConfig">儲存設定</button><small>標示需重啟的設定會在下次 daemon 啟動生效。</small></section><section><h2>Agent Workspace</h2><div class="files" id="files"></div><h3 id="filename">尚未選擇檔案</h3><textarea id="document"></textarea><button id="saveDocument">儲存文件</button></section><section><h2>Cron／Reminder</h2><div class="grid"><input id="scheduleName" placeholder="名稱"><select id="scheduleKind"><option value="cron">Cron</option><option value="once">一次性 Reminder</option></select><input id="scheduleWhen" placeholder="cron expression 或 ISO 時間"><input id="scheduleTimezone" value="Asia/Taipei" placeholder="Timezone"><select id="scheduleChannel"><option value="">不指定 Discord 頻道</option></select></div><textarea id="schedulePrompt" placeholder="Agent prompt" style="height:100px"></textarea><button id="createSchedule">建立</button><button id="refreshSchedules">重新整理</button><div id="schedules"></div></section><section><h2>內掛／外掛</h2><input id="pluginSource" placeholder="外掛 GitHub HTTPS URL 或本機路徑"><input id="pluginWorkspace" placeholder="monorepo workspace（可留空）"><button id="installPlugin">安裝外掛</button><button id="refreshPlugins">重新整理</button><small>內掛隨 ümiro 出貨，可停用但不可移除；外掛由使用者安裝，可獨立更新與移除。兩者都走相同 Plugin runtime。變更在重啟 daemon 後完整生效。</small><div id="plugins"></div></section><script src="app.js"></script></html>`;
const JS = `
const $=id=>document.getElementById(id);let file;let editingSchedule;let channelCatalog=new Map();let channelRefreshTimer;
...
const channelName=id=>{const x=channelCatalog.get(id);return x?x.guildName+' · '+(x.kind==='thread'&&x.parentName?x.parentName+' / ':'')+'#'+x.name:id||'未指定頻道'};
async function channels(){const items=await api('/api/channels');channelCatalog=new Map(items.map(x=>[x.id,x]));$('channels').replaceChildren(...items.map(x=>{const d=document.createElement('div');d.className='channel';const name=document.createElement('span');name.textContent=channelName(x.id);const id=document.createElement('small');id.textContent=' — '+x.id;d.append(name,id);return d}));const selected=$('scheduleChannel').value;$('scheduleChannel').replaceChildren(new Option('不指定 Discord 頻道',''),...items.map(x=>new Option(channelName(x.id)+' — '+x.id,x.id)));$('scheduleChannel').value=selected;await Promise.all([runs(),schedules()])}
async function schedules(){const items=await api('/api/schedules');$('schedules').replaceChildren(...items.map(x=>{const d=document.createElement('div');d.className='schedule';const destination=x.destination?.channelId?' — '+channelName(x.destination.channelId):'';const label=document.createElement('span');label.textContent=x.name+' — '+JSON.stringify(x.schedule)+destination+' — '+(x.enabled?'啟用':'停用');const toggle=document.createElement('button');toggle.textContent=x.enabled?'停用':'啟用';toggle.onclick=()=>api('/api/schedules/'+encodeURIComponent(x.id),{method:'PATCH',body:JSON.stringify({enabled:!x.enabled})}).then(schedules);const edit=document.createElement('button');edit.textContent='編輯';edit.onclick=()=>{editingSchedule=x.id;$('scheduleName').value=x.name;$('scheduleKind').value=x.schedule.kind;$('scheduleWhen').value=x.schedule.kind==='cron'?x.schedule.expression:x.schedule.at;$('scheduleTimezone').value=x.timezone;$('schedulePrompt').value=x.input?.prompt||'';$('scheduleChannel').value=x.destination?.channelId||'';$('createSchedule').textContent='儲存修改'};const remove=document.createElement('button');remove.textContent='刪除';remove.onclick=()=>api('/api/schedules/'+encodeURIComponent(x.id),{method:'DELETE'}).then(schedules).catch(e=>alert(e.message));d.append(label,toggle,edit,remove);return d}))}
