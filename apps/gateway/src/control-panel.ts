...
export interface ControlPanelOptions { readonly host: string; readonly port: number; readonly token: string; readonly configFile: string; readonly workspace: string; readonly schedules?: ControlPanelSchedules }
...
const HTML = `<!doctype html><html lang="zh-Hant"><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>ümiro 控制台</title><style>body{font:15px system-ui;max-width:1100px;margin:30px auto;padding:0 16px;background:#111;color:#eee}button,input,select,textarea{font:inherit}input,textarea,select{box-sizing:border-box;width:100%;background:#1d1d1d;color:#eee;border:1px solid #555;padding:8px}textarea{height:360px}button{width:auto;padding:8px 14px;margin:6px 4px 6px 0}section{border:1px solid #444;padding:16px;margin:16px 0}small{color:#aaa}.files button{font-size:13px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}.schedule{border-top:1px solid #444;padding:8px 0}</style><h1>ümiro V2 控制台</h1><section><label>Web UI Token<input id="token" type="password" autocomplete="off"></label><button id="connect">連線</button><span id="state"></span></section><section><h2>設定</h2><div id="help"></div><textarea id="config"></textarea><button id="saveConfig">儲存設定</button><small>標示需重啟的設定會在下次 daemon 啟動生效。</small></section><section><h2>Agent Workspace</h2><div class="files" id="files"></div><h3 id="filename">尚未選擇檔案</h3><textarea id="document"></textarea><button id="saveDocument">儲存文件</button></section><section><h2>Cron／Reminder</h2><div class="grid"><input id="scheduleName" placeholder="名稱"><select id="scheduleKind"><option value="cron">Cron</option><option value="once">一次性 Reminder</option></select><input id="scheduleWhen" placeholder="cron expression 或 ISO 時間"><input id="scheduleTimezone" value="Asia/Taipei" placeholder="Timezone"><input id="scheduleChannel" placeholder="Discord channel ID（可留空）"></div><textarea id="schedulePrompt" placeholder="Agent prompt" style="height:100px"></textarea><button id="createSchedule">建立</button><button id="refreshSchedules">重新整理</button><div id="schedules"></div></section><script src="/app.js"></script></html>`;
const JS = `const $=id=>document.getElementById(id);let file;const names=['SOUL.md','AGENT.md','OWNER.md','MEMORY.md','PEOPLE.md'];const headers=()=>({'authorization':'Bearer '+$('token').value,'content-type':'application/json'});async function api(path,options={}){const r=await fetch(path,{...options,headers:{...headers(),...(options.headers||{})}});const data=await r.json();if(!r.ok)throw new Error(data.error||r.statusText);return data}async function schedules(){const items=await api('/api/schedules');$('schedules').replaceChildren(...items.map(x=>{const d=document.createElement('div');d.className='schedule';const label=document.createElement('span');label.textContent=x.name+' — '+JSON.stringify(x.schedule)+' — '+(x.enabled?'啟用':'停用');const toggle=document.createElement('button');toggle.textContent=x.enabled?'停用':'啟用';toggle.onclick=()=>api('/api/schedules/'+encodeURIComponent(x.id),{method:'PATCH',body:JSON.stringify({enabled:!x.enabled})}).then(schedules);const remove=document.createElement('button');remove.textContent='刪除';remove.onclick=()=>confirm('確定刪除？')&&api('/api/schedules/'+encodeURIComponent(x.id),{method:'DELETE'}).then(schedules);d.append(label,toggle,remove);return d}))}async function connect(){localStorage.umiroToken=$('token').value;const [schema,config]=await Promise.all([api('/api/schema'),api('/api/config')]);$('help').innerHTML=Object.values(schema).map(x=>'<p><b>'+x.label+'</b> — '+x.description+(x.restartRequired?' <small>（需重啟）</small>':'')+'</p>').join('');$('config').value=JSON.stringify(config,null,2);await schedules();$('state').textContent='已連線'}async function load(name){const x=await api('/api/workspace/'+name);file=name;$('filename').textContent=name;$('document').value=x.content}names.forEach(n=>{const b=document.createElement('button');b.textContent=n;b.onclick=()=>load(n).catch(e=>alert(e.message));$('files').appendChild(b)});$('connect').onclick=()=>connect().catch(e=>$('state').textContent=e.message);$('saveConfig').onclick=()=>api('/api/config',{method:'PUT',body:$('config').value}).then(()=>alert('已儲存')).catch(e=>alert(e.message));$('saveDocument').onclick=()=>file?api('/api/workspace/'+file,{method:'PUT',body:JSON.stringify({content:$('document').value})}).then(()=>alert('已儲存')).catch(e=>alert(e.message)):alert('請先選檔案');$('refreshSchedules').onclick=()=>schedules().catch(e=>alert(e.message));$('createSchedule').onclick=()=>{const kind=$('scheduleKind').value;const when=$('scheduleWhen').value;api('/api/schedules',{method:'POST',body:JSON.stringify({name:$('scheduleName').value,kind,expression:kind==='cron'?when:undefined,at:kind==='once'?when:undefined,timezone:$('scheduleTimezone').value,prompt:$('schedulePrompt').value,channelId:$('scheduleChannel').value||undefined})}).then(schedules).catch(e=>alert(e.message))};$('token').value=localStorage.umiroToken||'';`;
