...
export const CONFIG_EXPLANATIONS = {
  ...
  modelCapabilities: { label: "模型能力", description: "模型明確支援的能力清單，例如 vision、function_tools、hosted_web_search；未宣告的 hosted 能力會在該 profile 明確拒絕。", restartRequired: true },
...
export class ControlPanelServer {
  ...
  private async handle(request: IncomingMessage, response: ServerResponse): Promise<void> {
    try {
      ...
      if (scheduleId && request.method === "PATCH") { if (!this.options.schedules) return json(response, 503, { error: "scheduler unavailable" }); const input = await body(request) as Record<string, unknown>; if (typeof input.enabled === "boolean" && Object.keys(input).length === 1) return json(response, 200, await this.options.schedules.setEnabled(scheduleId, input.enabled)); if (!this.options.schedules.update) return json(response, 503, { error: "schedule editing unavailable" }); if (typeof input.name !== "string" || !input.name.trim() || (input.kind !== "cron" && input.kind !== "once") || typeof input.timezone !== "string" || !input.timezone || typeof input.prompt !== "string" || !input.prompt.trim()) throw new TypeError("name, kind, timezone and prompt are required"); if (input.kind === "cron" && typeof input.expression !== "string") throw new TypeError("cron expression is required"); if (input.kind === "once" && typeof input.at !== "string") throw new TypeError("reminder time is required"); return json(response, 200, await this.options.schedules.update(scheduleId, { name: input.name.trim(), kind: input.kind, ...(input.kind === "cron" ? { expression: input.expression as string } : { at: input.at as string }), timezone: input.timezone, prompt: input.prompt, ...(typeof input.channelId === "string" && input.channelId ? { channelId: input.channelId } : {}) })); }
