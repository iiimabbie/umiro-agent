...
async function connect(){localStorage.umiroToken=$('token').value;const [schema,config,runtime]=await Promise.all([api('/api/schema'),api('/api/config'),api('/api/runtime')]);$('help').innerHTML=Object.values(schema).map(x=>'<p><b>'+x.label+'</b> — '+x.description+(x.restartRequired?' <small>（需重啟）</small>':'')+'</p>').join('');$('config').value=JSON.stringify(config,null,2);$('runtime').textContent=JSON.stringify(runtime,null,2);await Promise.all([schedules(),plugins(),approvals()]);$('state').textContent='已連線'}
