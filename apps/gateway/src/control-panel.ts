...
export interface ControlPanelOptions { readonly host: string; readonly port: number; readonly token: string; readonly configFile: string; readonly workspace: string; readonly schedules?: ControlPanelSchedules; readonly plugins?: ControlPanelPlugins }
...
async function plugins(){const items=await api('/api/plugins');$('plugins').replaceChildren(...items.map(x=>{const d=document.createElement('div');d.className='plugin';const label=document.createElement('span');label.textContent=(x.enabled?'啟用':'停用')+' — '+x.source;const toggle=document.createElement('button');toggle.textContent=x.enabled?'停用':'啟用';toggle.onclick=()=>pluginAction(x.enabled?'disable':'enable',x.source,x.workspace);const configure=document.createElement('button');configure.textContent='設定';configure.onclick=()=>{const value=prompt('Plugin JSON config',JSON.stringify(x.config||{},null,2));if(value!==null)pluginAction('configure',x.source,x.workspace,JSON.parse(value)).catch(e=>alert(e.message))};const update=document.createElement('button');update.textContent='更新';update.onclick=()=>pluginAction('update',x.source,x.workspace).catch(e=>alert(e.message));const remove=document.createElement('button');remove.textContent='移除';remove.onclick=()=>confirm('確定移除？')&&pluginAction('remove',x.source,x.workspace).catch(e=>alert(e.message));d.append(label,toggle,configure,update,remove);return d}))}
async function connect(){localStorage.umiroToken=$('token').value;const [schema,config]=await Promise.all([api('/api/schema'),api('/api/config')]);$('help').innerHTML=Object.values(schema).map(x=>'<p><b>'+x.label+'</b> — '+x.description+(x.restartRequired?' <small>（需重啟）</small>':'')+'</p>').join('');$('config').value=JSON.stringify(config,null,2);await Promise.all([schedules(),plugins()]);$('state').textContent='已連線'}
