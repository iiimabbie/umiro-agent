...
test("localhost control panel authenticates config and fixed workspace file operations", async () => {
  ...
  const schedules = [{ id: "schedule-1", name: "daily", enabled: true, schedule: { kind: "cron" as const, expression: "0 8 * * *" }, timezone: "Europe/London", nextFireAt: null, prompt: "提醒我", owner: { kind: "user" as const }, actions: { canToggle: true, canEdit: true, canDelete: true } }]; let created: unknown; let enabled: unknown; let updatedSchedule: unknown; let removed: unknown; let previewed: unknown; let restarted = false;
  let pluginAction: unknown; let runLimit: unknown; let runId: unknown; let conversationFilter: unknown; let messageArgs: unknown; let modelConnection: unknown; const audits: Array<{ event: string; data: Record<string, unknown> }> = [];
  const server = new ControlPanelServer({ host: "127.0.0.1", port: 0, token: "test-token", configFile, workspace, workspaceFiles: ["AGENT.md", "memory/FACTS.md"], secrets: () => ({ DISCORD_TOKEN: true, LLM_API_KEY: false }), models: async connection => { modelConnection = connection; return ["gemma4", "gpt-5"]; }, audit: (event, data) => audits.push({ event, data }), schedules: {
  ...
  }, plugins: { async list() { return [{ source: "builtin:memory", enabled: true }]; }, async run(...args) { pluginAction = args; return { ok: true }; } }, runs: { async list(limit) { runLimit = limit; return [{ id: "run-1", state: "succeeded", channelId: "channel-1" }]; }, async get(id) { runId = id; return id === "run-1" ? { run: { id } } : undefined; } }, channels: { async list() { return [{ id: "channel-1", name: "交誼廳", guildId: "guild-1", guildName: "測試站", kind: "channel" }]; }, async untrack(externalId) { if (externalId === "999999") throw Object.assign(new Error("這個頻道目前有執行中的對話，請等完成後再停止追蹤。"), { statusCode: 409 }); return externalId === "123456" ? { tracked: true, archivedConversationId: "conversation-1" } : { tracked: false }; } }, conversations: { async list(filter) { conversationFilter = filter; return [{ id: "conversation-1", state: "archived", scope: { transport: "discord", externalId: "channel-1", kind: "channel", name: "交誼廳" }, turnCount: 2 }]; }, async messages(id, limit, after) { messageArgs = [id, limit, after]; return id === "conversation-1" ? { conversation: { id }, messages: [{ turnId: "turn-1", sequence: 0, text: "hello", observed: true }], hasMore: false } : undefined; } }, logs: limit => [{ event: "test", limit }], usage: () => ({ completedRuns: 2, inputTokens: 10, outputTokens: 5 }), runtime: () => ({ status: "running", bot: { tag: "dev" } }), restart: () => { restarted = true; } }); await server.start();
  ...
  try {
    ...
    assert.equal(modelConnection, undefined);
    assert.deepEqual(await (await fetch(`${endpoint}/api/models/discover`, { method: "POST", headers, body: JSON.stringify({ baseUrl: "http://127.0.0.1:8317/v1", apiKey: "transient-key" }) })).json(), ["gemma4", "gpt-5"]);
    assert.deepEqual(modelConnection, { baseUrl: "http://127.0.0.1:8317/v1", apiKey: "transient-key" });
    assert.equal((await fetch(`${endpoint}/api/models/discover`, { method: "POST", headers, body: JSON.stringify({ baseUrl: "file:///tmp/models" }) })).status, 400);
    assert.equal((await fetch(`${endpoint}/api/models/discover`, { method: "POST", headers, body: JSON.stringify({ baseUrl: "http://127.0.0.1:8317/v1", extra: true }) })).status, 400);
    ...
    assert.equal((await fetch(`${endpoint}/api/channels/123456/tracking`, { method: "DELETE" })).status, 401);
    assert.deepEqual(await (await fetch(`${endpoint}/api/channels/123456/tracking`, { method: "DELETE", headers })).json(), { tracked: true, archivedConversationId: "conversation-1" });
    assert.equal((await fetch(`${endpoint}/api/channels/999999/tracking`, { method: "DELETE", headers })).status, 409);
    assert.equal((await fetch(`${endpoint}/api/channels/not-a-snowflake/tracking`, { method: "DELETE", headers })).status, 400);
    assert.equal((await fetch(`${endpoint}/api/channels/123456/tracking?unexpected=true`, { method: "DELETE", headers })).status, 400);
    ...
    assert.deepEqual(audits.map(item => item.event), ["control.channel.untracked", "control.config.saved", "control.workspace.saved", "control.workspace.saved", "control.schedule.created", "control.schedule.toggled", "control.schedule.updated", "control.schedule.removed", "control.plugin.action", "control.plugin.action", "control.runtime.restart"]);
...
test("plugin view routes are authenticated, writable, and bounded to IDs", async () => {
  const calls: string[] = [];
  let updatedContent = "";
  const server = new ControlPanelServer({ host: "127.0.0.1", port: 0, token: "secret", configFile: "/tmp/no", workspace: "/tmp", pluginViews: {
    list: () => [{ pluginId: "diary", id: "diary", title: "日記", kind: "markdown-collection" as const, writable: true }],
    ...
    async updateDocument(viewId, documentId, content) { if (content.length > 100_000) throw new TypeError("content too large"); calls.push(`update:${viewId}:${documentId}`); updatedContent = content; return { id: documentId, title: documentId, content }; },
  } });
  ...
  try {
    ...
    assert.deepEqual(await (await fetch(`${endpoint}/api/plugin-views`, { headers })).json(), [{ pluginId: "diary", id: "diary", title: "日記", kind: "markdown-collection", writable: true }]);
    ...
    const updated = await fetch(`${endpoint}/api/plugin-views/diary/documents/2026-09-21`, { method: "PUT", headers: { ...headers, "content-type": "application/json" }, body: JSON.stringify({ content: "# updated" }) });
    assert.equal(updated.status, 200);
    assert.deepEqual(await updated.json(), { id: "2026-09-21", title: "2026-09-21", content: "# updated" });
    assert.equal(updatedContent, "# updated");
    const oversized = await fetch(`${endpoint}/api/plugin-views/diary/documents/2026-09-21`, { method: "PUT", headers: { ...headers, "content-type": "application/json" }, body: JSON.stringify({ content: "x".repeat(100_001) }) });
    ...
    assert.deepEqual(calls, ["list:diary", "read:diary:2026-09-21", "read:diary:missing", "read:diary:explode", "update:diary:2026-09-21"]);
  } finally { await server.stop(); }
});
