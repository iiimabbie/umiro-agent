...
test("inbound images, text and unsupported files remain visible to the model", async () => {
  ...
  const artifact = (id: string, mediaType: string, location: string, filename: string): Artifact => ({ id, ownerPrincipalId: "p", visibility: "shared", mediaType, filename, size: 3, sha256: id, location, state: "stored", createdAt: "now", updatedAt: "now" });
  try {
