...
interface UmiroConfig { model: string; protocol?: "openai_responses" | "openai_chat_completions"; modelCapabilities?: string[]; profiles?: Record<string, { model: string; protocol?: "openai_responses" | "openai_chat_completions"; capabilities?: string[]; reasoningEffort?: string }>; contextMaxTokens?: number; pricing?: Record<string, { inputUsdPerMillion: number; outputUsdPerMillion: number }>; embedding?: Record<string, unknown>; skills?: string[]; discord?: Record<string, unknown>; conversation?: { autoArchive?: { enabled?: boolean; time?: string; timezone?: string } }; webUi?: Record<string, unknown>; plugins?: Array<{ path: string; config?: Record<string, unknown> }> }
...
async function embedding(action: string, provider?: string, model?: string, baseUrl?: string, requestsPerMinute?: string, recallLimit?: string, minSimilarity?: string, queryModel?: string, dimensionsValue?: string): Promise<void> {
  ...
  if ((queryModel === undefined) !== (dimensionsValue === undefined)) throw new Error("--query-model and --dimensions must be provided together");
  ...
  const dimensions = dimensionsValue === undefined ? undefined : Number(dimensionsValue);
  ...
  if (dimensions !== undefined && (!Number.isSafeInteger(dimensions) || dimensions < 1 || dimensions > 65_536)) throw new Error("--dimensions must be a safe integer between 1 and 65536");
  ...
  const next = { provider, model: model.trim(), ...(queryModel !== undefined ? { separateQueryModel: true, queryModel: queryModel.trim(), dimensions } : {}), ...tuning };
  if (queryModel !== undefined && !queryModel.trim()) throw new Error("--query-model must not be empty");
...
async function plugin(action: string, source?: string, workspaceName?: string, configJson?: string, removeSecrets = false): Promise<void> {
  ...
  console.log("automatic restart skipped; the user must run `umo restart` manually to reload plugin changes");
}
