...
test("Discord trigger policy is configurable through the installed CLI path", async () => {
  ...
  try {
    ...
    await exec(process.execPath, [cli, "discord", "configure", "--allowed-guilds", "g1,g2", "--allowed-channels", "c1", "--ambient-channels", "a1,a2", "--ignored-channels", "i1", "--respond-to-bots", "true", "--status", "idle", "--activity", "testing"], { env });
    const config = JSON.parse(await readFile(join(home, "config", "umiro.json"), "utf8")) as { discord: Record<string, unknown> };
    assert.deepEqual(config.discord, { ignoredChannels: ["i1"], ambientChannels: ["a1", "a2"], allowedChannels: ["c1"], allowedGuilds: ["g1", "g2"], respondToBots: true, presence: { status: "idle", activity: "testing" } });
