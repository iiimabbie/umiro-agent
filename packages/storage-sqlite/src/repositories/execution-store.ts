...
interface ResultRow {
  ...
  model_input_artifact_ids_json: string | null;
  completed_at: string;
}
...
export class SQLiteExecutionStore implements ExecutionStore, ConversationStore, ConversationIngressStore, ConversationPreferenceStore, DelegationStore, ArtifactStore, SearchDocumentProjection {
  ...
  async updateArtifactLocation(request: { readonly artifactId: string; readonly location: string; readonly filename?: string; readonly size: number; readonly sha256: string; readonly updatedAt: string }): Promise<void> {
    if (request.size < 0 || !/^[a-f0-9]{64}$/i.test(request.sha256) || !request.location) throw new TypeError("invalid artifact location metadata");
    const result = this.database.prepare("UPDATE artifacts SET location=?, filename=?, size=?, sha256=?, updated_at=? WHERE id=? AND state <> 'deleted'")
      .run(request.location, request.filename ?? null, request.size, request.sha256, request.updatedAt, request.artifactId);
    expectOne(result.changes, `artifact ${request.artifactId} not found or deleted`);
  }
  ...
  async recordOperationOutcome(operationId: string, result: OperationResult, updatedAt: string): Promise<void> {
    ...
    this.database.transaction(() => {
      ...
      this.database.prepare(`
        INSERT INTO operation_results(operation_id, outcome, effect_status, output_json, error_json, artifact_ids_json, model_input_artifact_ids_json, completed_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT(operation_id) DO UPDATE SET
          ...
          model_input_artifact_ids_json = excluded.model_input_artifact_ids_json,
      ...
      `).run(
        ...
        result.modelInputArtifactIds ? json(result.modelInputArtifactIds) : null,
        result.completedAt,
      );
  ...
  async getOperationResult(operationId: string): Promise<OperationResult | undefined> {
    ...
    return row ? {
      ...
      ...(row.model_input_artifact_ids_json ? { modelInputArtifactIds: parseJson<NonNullable<OperationResult["modelInputArtifactIds"]>>(row.model_input_artifact_ids_json) } : {}),
