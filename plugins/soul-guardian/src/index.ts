...
export function createPlugin(context: PluginSetupContext): PluginInstance {
  ...
  return {
    contributions: { tools: [
      ...
      tool("soul_guardian_approve", "Approve current contents as the new baseline.", CAP.approve, { type: "object", required: ["paths"], properties: { paths: { type: "array", items: { type: "string" } } } }, { interactionRequirement: "interactive_required", sideEffect: "idempotent" }, async input => { try { return ok(await service.approve(paths(input))); } catch (e) { return fail(e); } }),
      tool("soul_guardian_restore", "Restore monitored files from their approved baseline.", CAP.restore, { type: "object", required: ["paths"], properties: { paths: { type: "array", items: { type: "string" } } } }, { interactionRequirement: "interactive_required", sideEffect: "idempotent" }, async input => { try { return ok(await service.restore(paths(input))); } catch (e) { return fail(e); } }),
