...
export function createPlugin(setup: PluginSetupContext): PluginInstance {
  ...
  const tools: ToolDefinition[] = [
    ...
    define({ name: "read_file", description: "Read one file inside the configured agent workspace. For workspace/attachments images, PDFs, and supported documents, this loads the file into the next model turn; do not use cat or base64 for binary files.", inputSchema: { type: "object", additionalProperties: false, required: ["path"], properties: { path: { type: "string", minLength: 1 } } }, policy: { capability: "filesystem.read", tier: "privileged", interactionRequirement: "not_required", sideEffect: "none", resource: input => ({ kind: "workspace_path", id: String(input.path) }) }, async execute(input, context) {
