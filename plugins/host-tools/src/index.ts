...
export function createPlugin(setup: PluginSetupContext): PluginInstance {
  ...
  const define = (definition: Omit<ToolDefinition, "execute"> & { execute: (input: JsonObject, context: ToolExecutionContext) => Promise<unknown> }): ToolDefinition => ({ ...definition, async execute(input, context) { try { return ok(await definition.execute(input, context)); } catch (error) { return failed(error, definition.policy.sideEffect === "none" ? "not_applicable" : "unknown"); } } });
  const tools: ToolDefinition[] = [
    ...
    define({ name: "bash", description: "Execute a non-interactive shell command in the configured agent workspace (30 seconds, 1 MiB output).", inputSchema: { type: "object", additionalProperties: false, required: ["command"], properties: { command: { type: "string", minLength: 1, maxLength: 20000 } } }, policy: { capability: "shell.execute", tier: "privileged", interactionRequirement: "not_required", sideEffect: "non_idempotent", timeoutMs: 35_000 }, async execute(input, context) { return execute(String(input.command), root, context.signal); } }),
    define({ name: "web_fetch", description: "Fetch a public HTTP(S) URL with redirect, private-network and response-size guards.", inputSchema: { type: "object", additionalProperties: false, required: ["url"], properties: { url: { type: "string", minLength: 1 }, method: { enum: ["GET", "HEAD"] } } }, policy: { capability: "web.fetch", tier: "privileged", interactionRequirement: "not_required", sideEffect: "none", timeoutMs: 30_000, resource: input => ({ kind: "url", id: String(input.url) }) }, async execute(input, context) {
