...
export function createPlugin(setup: PluginSetupContext): PluginInstance {
  ...
  const define = (definition: Omit<ToolDefinition, "execute"> & { execute: (input: JsonObject, context: ToolExecutionContext) => Promise<unknown> }): ToolDefinition => ({ ...definition, async execute(input, context) { try { const value = await definition.execute(input, context); if (value && typeof value === "object" && "modelInputArtifactIds" in value && Array.isArray((value as ModelToolValue).modelInputArtifactIds)) return { ...ok((value as ModelToolValue).output, definition.policy.sideEffect === "none" ? "not_applicable" : "confirmed"), modelInputArtifactIds: (value as ModelToolValue).modelInputArtifactIds }; return ok(value, definition.policy.sideEffect === "none" ? "not_applicable" : "confirmed"); } catch (error) { return failed(error, error instanceof HostToolInputError || definition.policy.sideEffect === "none" ? "not_applicable" : "unknown"); } } });
  const tools: ToolDefinition[] = [
    ...
    define({ name: "read_file", description: "Read one file inside the configured agent workspace. Workspace/attachments images, PDFs, and supported documents are loaded into the next model turn as binary model input.", inputSchema: { type: "object", additionalProperties: false, required: ["path"], properties: { path: { type: "string", minLength: 1 } } }, policy: { capability: "filesystem.read", tier: "privileged", interactionRequirement: "not_required", sideEffect: "none", resource: input => ({ kind: "workspace_path", id: String(input.path) }) }, async execute(input, context) {
      const requested = String(input.path).startsWith(`workspace${sep}`) ? String(input.path).slice(`workspace${sep}`.length) : String(input.path);
      const path = await existingPath(root, requested); const relativePath = relative(root, path); const metadata = await stat(path); const limit = config.maxReadBytes ?? 1024 * 1024;
      if (!metadata.isFile()) throw new Error("file must be regular");
      const mediaType = workspaceMediaType(path);
      if (relativePath === "attachments" || relativePath.startsWith(`attachments${sep}`)) {
        if (!mediaType) throw new Error("unsupported workspace attachment type; only supported images, PDFs, and documents can be loaded for model input");
        if (metadata.size > 20 * 1024 * 1024) throw new Error("workspace attachment exceeds the model input size limit");
        const service = artifacts(); if (!service?.resolveWorkspaceFileForModel) throw new Error("artifact model input service is unavailable");
        const artifact = await service.resolveWorkspaceFileForModel({ sourcePath: relativePath, ownerPrincipalId: context.execution.actor.id, parentSource: { kind: "operation", id: context.operationId } });
        return { output: { path: relativePath, filename: basename(path), mediaType: artifact.mediaType, size: artifact.size, artifactId: artifact.id, loadedForModel: true }, modelInputArtifactIds: [artifact.id] } satisfies ModelToolValue;
      }
      if (metadata.size > limit) throw new Error(`file must be regular and no larger than ${limit} bytes`);
    ...
    define({ name: "bash", description: "Execute a non-interactive shell command in the configured agent workspace (30 seconds, 1 MiB output). Umiro lifecycle controls remain user-operated, so plugin changes remain pending until the user manually restarts it. Use move_file for safe attachment renames; use download_file for durable public downloads.", inputSchema: { type: "object", additionalProperties: false, required: ["command"], properties: { command: { type: "string", minLength: 1, maxLength: 20000 } } }, policy: { capability: "shell.execute", tier: "privileged", interactionRequirement: "not_required", sideEffect: "non_idempotent", timeoutMs: 35_000 }, async execute(input, context) { return execute(String(input.command), root, context.signal); } }),
    define({ name: "move_file", description: "Move one workspace attachment using attachments/... paths; use attachments/.trash/... for recoverable cleanup. Artifact resolution follows ordinary filesystem moves by hash.", inputSchema: { type: "object", additionalProperties: false, required: ["source", "destination"], properties: { source: { type: "string", minLength: 1 }, destination: { type: "string", minLength: 1 } } }, policy: { capability: "filesystem.write", tier: "privileged", interactionRequirement: "not_required", sideEffect: "idempotent", resource: input => ({ kind: "workspace_path", id: String(input.source) }) }, async execute(input) { const source = String(input.source); const destination = String(input.destination); attachmentPath(root, source); attachmentPath(root, destination); const service = artifacts(); if (!service) throw new Error("artifact service is unavailable"); return service.moveWorkspaceFile({ sourcePath: source, destinationPath: destination }); } }),
