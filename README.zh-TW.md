...
控制台的 Discord 頻道清單也提供「停止追蹤」。這會封存目前 conversation 並移除追蹤 scope，之後其他人的一般聊天不會再被記錄；既有 conversation、回覆、location、preferences、日記與搜尋資料都會保留。日後再次明確 @ Umiro 或回覆 Bot，即可重新建立追蹤。這個操作不會修改 `discord.ignoredChannels`；若連明確 trigger 也要阻擋，請另外設定 ignoredChannels。操作需要登入，頻道有 active Run 時會拒絕，等執行完成後再操作。
